...
def raw_to_session(session_ID, outlier_threshold=(99999, 99)):
    ...
    data = pd.read_csv(f"analysis/data/{session_ID}/raw.csv", parse_dates=["Datetime"])
    data.sort_values("Datetime", inplace=True)

    # Identify runs (start and end indices)
    start_indices = data[data["Action"] == "start"].index
    end_indices = data[data["Action"] == "end"].index
    runs = list(zip(start_indices, end_indices))

    # Analyze each run
    output_data = []
    for start, end in runs:
        run_data = data.iloc[start : end + 1]
        start_time = run_data.iloc[0]["Datetime"]
        end_time = run_data.iloc[-1]["Datetime"]
        total_time = (end_time - start_time).total_seconds() * 1000
        total_mistakes = run_data.iloc[-2]["Mistakes"]

        run_data.reset_index(drop=True, inplace=True)

        # Calculate Layout Difficulty
        key_releases = []
        for event_index, run_row in run_data.iterrows():
            if event_index > 0:
                if (
                    run_data.iloc[event_index - 1]["Mistakes"] == run_row["Mistakes"]
                    and run_row["Action"] == "released"
                ):
        ...
        layout_difficulty = (
            distances_sum / (len(key_releases) - 1) if len(key_releases) > 1 else 0
        )
    ...
    # Convert to DataFrame and filter out outliers
    output_df = pd.DataFrame(output_data)
    output_df = output_df[
        (output_df["Total Time Taken (ms)"] < outlier_threshold[0])
        & (output_df["Total Mistakes"] < outlier_threshold[1])
    ]
    ...
    # Save to CSV
    output_csv_path = os.path.join(f"analysis/data/{session_ID}/", "session.csv")
